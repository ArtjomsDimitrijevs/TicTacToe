...
class GameProgress(private val listener: GameProgressListener) : IGameBoard by GameBoard() {
    ...
    fun checkWinner(): Player {
        ...
        for (i in 0..2) {
            if (b[i][0] != Player.NONE && b[i][0] == b[i][1] && b[i][1] == b[i][2]) return b[i][0]  // check horizontal lines
            if (b[0][i] != Player.NONE && b[0][i] == b[1][i] && b[1][i] == b[2][i]) return b[0][i]  // check vertical lines
        }
